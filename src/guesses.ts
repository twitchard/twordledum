export const makeCalculateColor = (
	...
	const isMisplaced = (i: number, j: number): boolean =>
		!isCorrect(i, j) &&
		secretWordLetters.includes(guesses[i][j]) &&
		guesses[i][j] !== secretWordLetters[j];
